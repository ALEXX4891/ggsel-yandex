...
import GenerateUnitedReturnsRequest from '../model/GenerateUnitedReturnsRequest';
...
import ReportLanguageType from '../model/ReportLanguageType';
...
export default class DbsApi {
    ...
    */
     ...
     * @param {Number} campaignId Идентификатор магазина в кабинете.  Чтобы узнать идентификаторы своих магазинов, воспользуйтесь запросом [GET campaigns](../../reference/campaigns/getCampaigns.md).  ℹ️ [Что такое кабинет и магазин на Маркете](https://yandex.ru/support/marketplace/account/introduction.html) 
     * @param {Array.<Number>} ids Список идентификаторов лицензий для удаления.
     ...
     * data is of type: {@link module:model/EmptyApiResponse}
     */
    deleteOutletLicenses(campaignId, ids, callback) {
      let postBody = null;
      // verify the required parameter 'campaignId' is set
      if (campaignId === undefined || campaignId === null) {
      ...
      if (ids === undefined || ids === null) {
        throw new Error("Missing the required parameter 'ids' when calling deleteOutletLicenses");
      }
     ...
     * @callback module:api/DbsApi~generateGoodsRealizationReportCallback
     * @param {String} error Error message, if any.
     * @param {module:model/GenerateReportResponse} data The data returned by the service call.
     * @param {String} response The complete HTTP response.
     */
     ...
     * @param {Object} opts Optional parameters
     * @param {module:model/ReportFormatType} [format] Формат отчета.
     * @param {module:api/DbsApi~generateGoodsRealizationReportCallback} callback The callback function, accepting three arguments: error, data, response
     * data is of type: {@link module:model/GenerateReportResponse}
     */
     ...
     * @param {Object} opts Optional parameters
     * @param {module:model/ReportFormatType} [format] Формат отчета.
     * @param {module:model/ReportLanguageType} [language] Язык отчета.
    ...
    /**
     * Callback function to receive the result of the generateUnitedReturnsReport operation.
     * @callback module:api/DbsApi~generateUnitedReturnsReportCallback
     * @param {String} error Error message, if any.
     * @param {module:model/GenerateReportResponse} data The data returned by the service call.
     * @param {String} response The complete HTTP response.
     */
     ...
     * @param {module:model/GenerateUnitedReturnsRequest} generateUnitedReturnsRequest 
     * @param {Object} opts Optional parameters
     * @param {module:model/ReportFormatType} [format] Формат отчета.
     * @param {module:api/DbsApi~generateUnitedReturnsReportCallback} callback The callback function, accepting three arguments: error, data, response
     * data is of type: {@link module:model/GenerateReportResponse}
     */
    generateUnitedReturnsReport(generateUnitedReturnsRequest, opts, callback) {
      opts = opts || {};
      let postBody = generateUnitedReturnsRequest;
      // verify the required parameter 'generateUnitedReturnsRequest' is set
      if (generateUnitedReturnsRequest === undefined || generateUnitedReturnsRequest === null) {
        throw new Error("Missing the required parameter 'generateUnitedReturnsRequest' when calling generateUnitedReturnsReport");
      }
      ...
      let returnType = GenerateReportResponse;
      return this.apiClient.callApi(
        '/reports/united-returns/generate', 'POST',
        pathParams, queryParams, headerParams, formParams, postBody,
        authNames, contentTypes, accepts, returnType, null, callback
      );
    }
     ...
     * @param {Number} [chunk] Номер сегмента с результатами.  Значение по умолчанию: `0`.  {% note info \"Как запрашиваются номера сегментов\" %}  Последовательно, пока не будет получен сегмент с пустым ответом. Пустой ответ означает, что все предложения магазина получены.  {% endnote %}  {% note alert \"Нумерация начинается с 0\" %}  Чтобы запросить первую страницу, необходимо указать `chunk=0` и т. д.  {% endnote %} 
     ...
     * @param {Date} [publishedTimeTo] Конечная дата. Используется для фильтрации записей — по дате и времени публикации предложений на Яндекс Маркете.  Формат даты: ISO 8601 со смещением относительно UTC. Например, `2017-11-31T00:42:42+03:00`.  Значение по умолчанию: дата и время отправки запроса.  {% note info \"Вы не получите часть данных\" %}  Если во время переключения между страницами выходных данных на Яндекс Маркете появятся новые результаты индексации прайс-листа. Чтобы этого не произошло, зафиксируйте выходные данные с помощью входного параметра `published_time_to`. Значение параметра не должно быть датой из будущего.  {% endnote %} 
     ...
     * @param {Array.<String>} [vendor] Фильтрация по бренду товара.  Можно указать несколько брендов в одном параметре, через запятую, или в нескольких одинаковых параметрах. Например:  ```text translate=no ...vendor=Aqua%20Minerale,Borjomi... ...vendor=Aqua%20Minerale&vendor=Borjomi... ```  Чтобы товар попал в результаты фильтрации, его бренд должен точно совпадать с одним из указанных в запросе. Например, если указан бренд Schwarzkopf, то в результатах не будет товаров Schwarzkopf Professional.  Если в названии бренда есть символы, которые не входят в таблицу ASCII (в том числе кириллические символы), используйте для них URL-кодирование. Например, пробел — %20, апостроф «'» — %27 и т. д. Подробнее см. в разделе [Кодирование URL русскоязычной Википедии](https://ru.wikipedia.org/wiki/URL#Кодирование_URL).  В запросе можно указать либо параметр shopSku, либо любые параметры для фильтрации товаров. Совместное использование параметра shopSku и параметров для фильтрации приведет к ошибке. 
     * @param {String} [pageToken] Идентификатор страницы c результатами.  Если параметр не указан, возвращается первая страница.  Рекомендуется передавать значение выходного параметра `nextPageToken`, полученное при последнем запросе.  Если задан `page_token` и в запросе есть параметры `page_number` и `page_size`, они игнорируются. 
     * @param {Number} [limit] Количество значений на одной странице. 
     ...
     * @param {module:model/CurrencyType} [currency] Валюта, в которой указана цена предложения.  Возможные значения:  * `RUR` — российский рубль.  * `UAH` — украинская гривна.  * `BYR` — белорусский рубль.  * `KZT` — казахстанский тенге.  * `UZS` — узбекский сум. 
     ...
     * @param {Date} [toDate] Конечная дата для фильтрации заказов по дате оформления.  Показываются заказы, созданные до 00:00 указанного дня.  Формат даты: `ДД-ММ-ГГГГ`.  Между начальной (параметр `fromDate`) и конечной датой должно быть не больше 30 дней.  Значение по умолчанию: текущая дата.  Если промежуток времени между `toDate` и `fromDate` меньше суток, то `toDate` равен `fromDate` + сутки. 
     ...
     * @param {Boolean} [fake = false)] Фильтрация заказов по типам:  * `false` — настоящий заказ покупателя.  * `true` — [тестовый](../../concepts/sandbox.md) заказ Маркета. 
     * @param {Boolean} [hasCis = false)] Нужно ли вернуть только те заказы, в составе которых есть хотя бы один товар с кодом идентификации в системе [«Честный ЗНАК»](https://честныйзнак.рф/) или [«ASL BELGISI»](https://aslbelgisi.uz) (для продавцов Market Yandex Go):  * `true` — да.  * `false` — нет.  Такие коды присваиваются товарам, которые подлежат маркировке и относятся к определенным категориям. 
     ...
     * @param {Number} [regionId2] {% note warning \"Этот параметр устарел\" %}  Вместо него используйте `region_id`.  {% endnote %} 
     ...
     * @callback module:api/DbsApi~getPagedWarehousesCallback
     * @param {String} error Error message, if any.
     ...
     * @param {module:api/DbsApi~getPagedWarehousesCallback} callback The callback function, accepting three arguments: error, data, response
     ...
     * @callback module:api/DbsApi~getRegionsCodesCallback
     * @param {String} error Error message, if any.
     ...
     * @param {module:api/DbsApi~getRegionsCodesCallback} callback The callback function, accepting three arguments: error, data, response
     ...
     * @callback module:api/DbsApi~updateWarehouseStatusCallback
     * @param {String} error Error message, if any.
     ...
     * @param {module:api/DbsApi~updateWarehouseStatusCallback} callback The callback function, accepting three arguments: error, data, response
